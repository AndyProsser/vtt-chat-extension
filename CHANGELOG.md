...
### Added
- Manual **SYNC button** (refresh SVG icon) injected on D&D Beyond character sheet pages, placed next to the INFO button; clicking it forces a full character data sync to the VTT-Chat backend. SVG spins while the sync is in progress.
- `@keyframes vtt-spin` style tag injected once per page to drive the spin animation without a separate stylesheet.

### Removed
- `webRequest` permission removed from all three manifests (base, Chrome, Firefox) — no longer needed now that sync is user-initiated rather than XHR-triggered.
